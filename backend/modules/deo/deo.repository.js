...
async function getCompletedMeetings() {
  const result = await pool.query(
    `SELECT m.id, m.request_id, m.title, m.purpose, m.status, m.created_at, m.completed_at, m.completion_note, m.admin_comments, m.scheduled_at, m.scheduled_location, c.first_name, c.last_name, c.citizen_id, c.mobile_number, c.email
     FROM meetings m
     JOIN citizens c ON c.id = m.citizen_id
