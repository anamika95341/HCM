...
function FilesSection({ item, C, tone, defaultTab = "photos", hideTabBar = false }) {
  const [activeTab, setActiveTab] = useState(defaultTab);
  const [files, setFiles] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  ...
  useEffect(() => {
    // activeTab ko sync karo agar parent se defaultTab change ho
  ...
  useEffect(() => {
    ...
    if (item?.sourceId) loadFiles();
    else { setFiles([]); setLoading(false); }
    return () => { mounted = false; };
  }, [item]);

  const filesByTab = useMemo(() => ({
    photos: files.filter((f) => f.fileCategory === "image"),
    videos: files.filter((f) => f.fileCategory === "video"),
    documents: files.filter((f) => f.fileCategory === "document"),
  }), [files]);
  ...
  return (
    <div>
      {/* ── Tab bar — sirf tab bar show karo agar hideTabBar false ho ── */}
      {!hideTabBar && (
        <div style={{ display: "flex", gap: 4, marginBottom: 16, borderBottom: `1px solid ${C.border}`, paddingBottom: 12 }}>
          {FILE_TABS.map(({ id, label }) => {
            const active = activeTab === id;
            ...
            return (
              ...
              >
                {id === "photos" ? <ImageIcon size={13} /> : id === "videos" ? <Film size={13} /> : <FileText size={13} />}
                {label}
                <span style={{ fontSize: 10, fontWeight: 700, padding: "1px 6px", borderRadius: 99, background: active ? `${tone}20` : `${C.t3}15`, color: active ? tone : C.t3 }}>
                  {count}
                </span>
              </button>
            );
      ...
      {/* File grid */}
      {loading ? (
        <div style={{ color: C.t3, fontSize: 13, textAlign: "center", padding: "16px 0" }}>Loading files...</div>
      ) : filesForTab.length === 0 ? (
        <div style={{ color: C.t3, fontSize: 13, textAlign: "center", padding: "16px 0" }}>
          No {activeTab} attached to this {getItemKind(item)}.
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 12 }}>
          {filesForTab.map((file) => (
            <button
              ...
              type="button"
              onClick={() => handleDownload(file.id)}
              style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 10, background: C.bgElevated, cursor: "pointer", transition: "border-color 0.15s ease", textAlign: "left" }}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = `${tone}40`)}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = C.border)}
            >
              <FilePlaceholderIcon type={activeTab} tone={tone} />
              <div style={{ marginTop: 8, fontSize: 11, fontWeight: 600, color: C.t1, lineHeight: 1.4, wordBreak: "break-word", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                {file.name}
              </div>
              <div style={{ marginTop: 4, fontSize: 10, color: C.t3 }}>{formatFileSize(file.size)}</div>
            </button>
          ))}
        </div>
      )}
      ...
      {error && (
        <div style={{ marginTop: 14, padding: "8px 12px", borderRadius: 8, background: `${C.danger}12`, fontSize: 11, color: C.danger }}>
          {error}
        </div>
      )}
    </div>
...
function MeetingDetailModal({ meeting, onClose }) {
  ...
  useEffect(() => {
    ...
    async function loadCounts() {
      try {
        ...
        if (mounted) {
          setFileCounts({
            photos: result.filter((f) => f.fileCategory === "image").length,
            videos: result.filter((f) => f.fileCategory === "video").length,
            documents: result.filter((f) => f.fileCategory === "document").length,
          });
        }
      } catch {}
    }
  ...
  function getTabCount(id) {
    if (id === "photos") return fileCounts.photos;
    if (id === "videos") return fileCounts.videos;
    if (id === "documents") return fileCounts.documents;
    return null;
  }
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          zIndex: 60,
          animation: `${isClosing ? "overlayOut" : "overlayIn"} 0.24s ease forwards`,
        }}
      />
      ...
      >
        <div
          ...
          style={{
            ...
            pointerEvents: "all",
            animation: `${isClosing ? "modalOut" : "modalIn"} 0.28s cubic-bezier(0.4, 0, 0.2, 1) forwards`,
          }}
        >
          {/* ── Header ── */}
          <div style={{ padding: "20px 24px", background: C.bgElevated, borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {MODAL_TABS.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={(e) => { if (!isActive) { e.currentTarget.style.background = `${C.t3}10`; e.currentTarget.style.color = C.t1; } }}
                    onMouseLeave={(e) => { if (!isActive) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = C.t3; } }}
                  >
                    <Icon size={14} />
                    {label}
                    {/* Count badge — sirf file tabs pe */}
                    {count !== null && (
                      <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 6px", borderRadius: 99, background: isActive ? `${tone}20` : `${C.t3}15`, color: isActive ? tone : C.t3, minWidth: 18, textAlign: "center" }}>
                        {count}
                      </span>
                    )}
                  </button>
