...
export default function AdminVerifyPage() {
  ...
  const [form, setForm] = useState({ usernameOrEmail: "", otp: "" });
  ...
  async function handleVerify(event) {
    ...
    try {
      ...
      setError(toSafeUserMessage(requestError, "Unable to verify admin account"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="portal-content" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px", background: C.bg }}>
      <div style={{ width: "100%", maxWidth: 460, background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: "var(--portal-shadow)", overflow: "hidden" }}>
        ...
        <div style={{ padding: 32 }}>
          {success ? <MessageBox color={C.mint} bg={C.mintDim} icon={<CheckCircle size={18} />} message={success} /> : null}
          {error ? <MessageBox color={C.danger} bg={`${C.danger}20`} icon={<AlertCircle size={18} />} message={error} /> : null}
          ...
          <form onSubmit={handleVerify} style={{ display: "grid", gap: 18 }}>
            <Field label="Username or Email" icon={<Mail size={16} color={C.t3} />}>
              <input
                ...
                onChange={(event) => setForm((current) => ({ ...current, usernameOrEmail: normalizeInputText(event.target.value, { maxLength: 160, trim: false }) }))}
                required
                placeholder="admin.user or admin@gov.in"
                style={fieldStyle(C)}
              />
            </Field>
            ...
            <button type="button" disabled={loading || !form.usernameOrEmail} onClick={handleResendCode} style={secondaryTextButtonStyle(C)}>
              Resend Verification Code
            </button>

            <button type="button" onClick={() => navigate(PATHS.adminLogin)} style={secondaryButtonStyle(C)}>
              Back to Login
            </button>
          </form>
        </div>
...
function MessageBox({ color, bg, icon, message }) {
  return (
    <div style={{ marginBottom: 18, padding: "12px 14px", borderRadius: 10, border: `1px solid ${color}33`, background: bg, color, display: "flex", alignItems: "flex-start", gap: 10, fontSize: 12, lineHeight: 1.6 }}>
...
function secondaryTextButtonStyle(C) {
  return { background: "transparent", border: "none", color: C.purple, fontSize: 12, fontWeight: 600, cursor: "pointer", padding: 0 };
}
