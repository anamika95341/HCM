    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <h3 style={{ fontSize: 24, fontWeight: 700, color: C.t1, margin: "16px 0 0 0", lineHeight: 1.3, wordBreak: "break-word" }}>
                  {item.title}
                </h3>
                <div style={{ fontSize: 14, color: C.t2, marginTop: 12, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span>{new Date(item.startsAt).toLocaleDateString()}</span>
                  <span>•</span>
                  <span style={{ fontWeight: 500 }}>{formatTime(item.startsAt)}</span>
                  <span>-</span>
                  <span style={{ fontWeight: 500 }}>{formatTime(item.endsAt)}</span>
                </div>
              </div>
              <button
                type="button"
                ...
                onMouseEnter={(e) => { e.currentTarget.style.background = C.bg; e.currentTarget.style.color = C.t1; }}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "24px 28px" }}>
            {mode === "details" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: C.t3, textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 10, margin: 0 }}>Source</p>
                    <p style={{ fontSize: 15, color: C.t1, fontWeight: 600, margin: 0 }}>{item.source}</p>
                  </div>
                </div>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: C.t3, textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 10, margin: 0 }}>Description</p>
                  <p style={{ color: C.t2, lineHeight: 1.7, fontSize: 14, margin: 0 }}>{item.details}</p>
                </div>
              </div>
            )}
            ...
            {mode === "edit" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.t2, marginBottom: 8 }}>Description</label>
                  <textarea
                    value={editForm.details}
                    onChange={(event) => setEditForm((current) => ({ ...current, details: event.target.value }))}
                    rows={4}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.t2, marginBottom: 8 }}>Start Time</label>
                    <input
                      type="time"
                      value={editForm.startTime}
                      onChange={(event) => setEditForm((current) => ({ ...current, startTime: event.target.value }))}
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.t2, marginBottom: 8 }}>End Time</label>
                    <input
                      type="time"
                      value={editForm.endTime}
                      onChange={(event) => setEditForm((current) => ({ ...current, endTime: event.target.value }))}
          ...
          {/* Fixed Footer - Only show on Edit mode */}
          {mode === "edit" && (
            <div
              style={{
            ...
            >
              <button
                ...
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.t2; }}
...
export default function Calendar() {
  ...
  const visibleUpcomingItems = useMemo(() => {
    ...
    return normalizedItems.filter((item) => item.startDate >= today && item.startDate <= futureEnd);
  }, [normalizedItems]);

  const viewLabel = () => {
    if (view === "day") return cursorDate.toLocaleDateString(undefined, { day: "numeric", month: "long", year: "numeric" });
    if (view === "week") {
      const start = startOfWeek(cursorDate);
      const end = addDays(start, 6);
      return `${start.toLocaleDateString(undefined, { day: "numeric", month: "short" })} – ${end.toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}`;
  ...
  const rowsCount = Math.ceil(monthGrid.length / 7);
  ...
  return (
    ...
    >
      {error && (
        <div style={{ margin: "24px 32px 0 32px", padding: 14, borderRadius: 10, fontWeight: 500, background: `${C.danger}12`, border: `1px solid ${C.danger}33`, color: C.danger, fontSize: 13 }}>
          {error}
        </div>
      )}
        ...
        >
          ...
          <div
            style={{
              ...
              transition: "all 0.35s cubic-bezier(0.4, 0, 0.2, 1)", // 🔥 ADD THIS
            }}
          >
            {/* Header */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexShrink: 0 }}>
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: C.t1, margin: 0, marginBottom: 4 }}>
                  {viewLabel()}
                </h2>
                <p style={{ fontSize: 13, color: C.t3, margin: 0 }}>
                  {filteredItems.length} event{filteredItems.length !== 1 ? "s" : ""} in view
                </p>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <button
                    type="button"
                    onClick={() => { setCursorDate(startOfDay(new Date())); setSelectedDateItems(null); }}
                    style={{
                      padding: "7px 16px", borderRadius: 8,
                      border: `1px solid ${C.border}`, background: C.card,
                      color: C.t1, fontSize: 13, fontWeight: 500, cursor: "pointer",
                      transition: "all 0.15s ease",
                    }}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.purple; e.currentTarget.style.color = C.purple; }}
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: isSelected ? `${C.purple}06` : inMonth ? C.card : `${C.t3}04`,
                          padding: 10,
                          display: "flex",
                          flexDirection: "column",
                          cursor: "pointer",
            ...
            {/* ── Week View ── */}
            {view === "week" && (
              <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 10, overflow: "hidden" }}>
                {weekDays.map((day) => {
                  const isToday = isSameDay(day.date, new Date());
                  const isSelected = selectedDateItems?.date && isSameDay(day.date, selectedDateItems.date);
                  ...
                  return (
                    <div
                      ...
                      style={{
                        border: `1px solid ${isSelected ? C.purple : C.border}`,
                        borderRadius: 10,
                        background: isSelected ? `${C.purple}06` : C.card,
                        padding: 12,
                        cursor: "pointer",
                        transition: "all 0.15s ease",
                        display: "flex",
                    ...
                    return (
                      <button
                        ...
                        onMouseEnter={(e) => {
                          ...
                          e.currentTarget.style.background = `${tone}04`;
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.borderColor = C.border;
                          e.currentTarget.style.background = C.card;
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexShrink: 0 }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 600, color: C.t3, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4 }}>
                  Upcoming
                </div>
                  ...
                  {visibleUpcomingItems.slice(0, 5).map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Title */}
                        <div style={{ fontSize: 13, fontWeight: 600, color: C.t1, marginBottom: 6, lineHeight: 1.3 }}>
                          {item.title}
                        </div>

                        {/* Date & Time */}
                        <div style={{ fontSize: 12, color: tone, fontWeight: 500 }}>
                          {new Date(item.startsAt).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}
